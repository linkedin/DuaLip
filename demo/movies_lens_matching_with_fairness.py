...
from dualip.objectives.matching_with_fairness import MatchingInputArgs
from dualip.projections.base import create_projection_map
from dualip.run_solver import run_solver


@dataclass
class MovielensMatchingWithFairnessConfig:
...
def prepare_movielens_matching_with_fairness(config: MovielensMatchingWithFairnessConfig) -> Tuple[MatchingInputArgs, Dict[str, int], Dict[int, int]]:
    """
    Convert MovieLens ratings to MatchingInputArgs consumable by fairness matching objective.
    ...
    torch.save(
        {
            ...
            "group_ratio": input_args.group_ratio,
        },
        f"{out_prefix}.pt",
    )
...
def _load_snapshot(
    ...
    group_ratio = float(payload.get("group_ratio", 0.2))
...
def main():
    parser = argparse.ArgumentParser(description="Build matching-with-fairness inputs from MovieLens ratings.")
    ...
    parser.add_argument("--group_ratio", type=float, default=0.2)
    ...
    if args.in_prefix:
        ...
        config = MovielensMatchingWithFairnessConfig(
        ...
        input_args, user_map, row_to_movie = prepare_movielens_matching_with_fairness(config)
    ...
    if args.run_solver:
        ...
        objective_args = ObjectiveArgs(objective_type="matching_with_fairness")
        result = run_solver(
        ...
        print("Prepared matching-with-fairness inputs.")
        print("A shape:", tuple(input_args.A.shape), "C shape:", tuple(input_args.c.shape), "b shape:", tuple(input_args.b_vec.shape))
