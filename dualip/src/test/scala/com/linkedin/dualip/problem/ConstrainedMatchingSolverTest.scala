...
class ConstrainedMatchingSolverTest {
  ...
  def checkConsistencyLambda(lambda: BSV[Double], expectedLambda: BSV[Double]): Unit = {
    (0 until lambda.length).foreach { i =>
      Assert.assertTrue(Math.abs(lambda(i) - expectedLambda(i)) < tolerance)
    }
  }

  def checkConsistency(value: DualPrimalDifferentiableComputationResult): Unit = {
    checkConsistencyLambda(value.lambda, BSV(expectedDuals.lambdaLocal.toArray ++ expectedDuals.lambdaGlobal.toArray))
  ...
  def testPrimal(): Unit = {
    ...
    val value = f.calculate(BSV(expectedDuals.lambdaLocal.toArray ++ expectedDuals.lambdaGlobal.toArray),
      mutable.Map.empty, 1)
  ...
  def testBoxCutSolver(): Unit = {
    ...
    val optimizer = new AcceleratedGradientDescent(maxIter = 5000, dualTolerance = dualTolerance)
